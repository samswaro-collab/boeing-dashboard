    ...
    steps:
      ...
      - name: Generate data_boeing.json
        ...
        run: |
          ...
          BASELINE = {
            'bca': 921, 'bgs': 50, 'bds': 75, 'spirit': 107,
            'bietc': 145, 'betc': 30, 'wisk': 52, 'avionx': 10,
            'aurora': 60, 'insitu': 50,
          }

          def fetch_count(keyword):
            url = ('https://jobs.boeing.com/api/apply/v2/jobs?domain=boeing.com'
                   '&keyword=' + urllib.parse.quote(keyword) + '&start=0&num=1')
            try:
              ...
              print('fetch failed for', keyword, ':', e)
              return None
          ...
          print('data_boeing.json written — total roles:', total)
          print('lastRefreshed:', today)
